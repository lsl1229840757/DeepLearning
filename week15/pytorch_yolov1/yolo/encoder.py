...
def yolo_encoder(box_list,ceil_size,box_num,cls_num):
    ...
    for gt,l in zip(box_list.box,labels):
        local_x = min(int(round((gt[2] + gt[0]) / 2)),w-1)  # 不能超过边界
        local_y = min(int(round((gt[3] + gt[1]) / 2)),h-1)

        for j in range(box_num):
            bb_response[j, local_y, local_x] = 1  # 决定该bbox是否对该obj负责
            ...
            bb_boxes[j * 4 + 3, local_y, local_x] = np.sqrt(max((gt[3] - gt[1]),0.01))  # 这四步在记录中点和w,h
