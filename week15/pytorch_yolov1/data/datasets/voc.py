...
class VOCDatasets(data.Dataset):
    def __init__(self, list_file,train=False):
        ...
        self.grid = Grid(True, True, rotate=1,offset= 0,ratio= 0.5, mode=1, prob=0.7)  # TODO 好像是数据增强用的
    ...
    def _get_label(self, file, size):
        ...
        for f in tmp.readlines():
            ...
            difficult.append(0)  # 这里直接添加0, 是因为前面制作txt的时候就直接忽略了困难的物体
        tmp.close()
    ...
    def _get_img(self, img_file):
        img = cv2.imread(img_file)[:, :, ::-1].copy()  # 将bgr通道转为rgb
        return img
    ...
    def get_data(self, idx):
        ...
        gt_list = self._get_label(gt_path, (img.shape[1], img.shape[0]))  # 这里会返回一个BoxList类

        if self.train:  # 如果是训练模式就需要用到数据增强
    ...
    def _data_aug(self,img,gt_list):
        ...
        if random.random() > 0.5:
            ...
            img = img.permute((2, 0, 1))  # pytorch里面是channel first

            img, label = self.grid(img, gt_list)
            img = img.permute((1, 2, 0))  # 变换回channel end
            img = img * 255
            img = img.numpy()
            img = img.astype(np.uint8)  # 处理为255的图像
    ...
    def __getitem__(self, idx):
        meta = self.get_data(idx)  # meta中储存了boxlist(gt,labels), img, img_width, img_height
        return meta
