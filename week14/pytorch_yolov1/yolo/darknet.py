...
layer_configs = [
    # Unit1 (2)
    (32, 3, True),  # outplane, kernel_size, pool
    (64, 3, True),
    # Unit2 (3)
    (128, 3, False),
    (64, 1, False),
...
def load_conv_bn(buf, start, conv_model, bn_model):
    ...
    bn_model.bias.data.copy_(torch.from_numpy(buf[start:start + num_b]))  # 这种in-place操作, 需要用到data或者detach()
    start = start + num_b
...
class darknet_19(nn.Module):
    ...
    def load_weight(self, weight_file):
        ...
        header = np.fromfile(fp, count=4, dtype=np.int32)  # darknet的权重文件是np读取,
        header = torch.from_numpy(header)
        buf = np.fromfile(fp, dtype=np.float32)  # 剩下的全部是float32

        start = 0
        for idx, m in enumerate(self.feature.modules()):  # 这个module会返回包括Sequential和Sequential内部的module
